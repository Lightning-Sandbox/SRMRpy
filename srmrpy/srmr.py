...
from srmrpy.modulation_filters import compute_modulation_cfs, modfilt, modulation_filterbank
...
def main():
    ...
    parser.add_argument(
        "--mincf", dest="min_cf", type=float, default=4.0, help="Center frequency of the first modulation filter"
    )
    parser.add_argument(
        "--maxcf", dest="max_cf", type=float, default=128.0, help="Center frequency of the last modulation filter"
    )
    parser.add_argument(
        "path", metavar="path", nargs="+", help="Path of the file or files to be processed. Can also be a folder."
    )
